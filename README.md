...
There are two versions of the binary: `-x86_64-unknown-linux-gnu` (dynamically
linked) and `-x86_64-unknown-linux-musl` (statically linked). The first uses the
GNU C library (glibc), while the second uses the musl C library (musl libc). In
a nutshell, `-x86_64-unknown-linux-gnu` is more compatible with various Linux
distributions due to its reliance on the widely used glibc. At the same time,
`x86_64-unknown-linux-musl` produces more portable and self-contained binaries
because it includes all necessary libraries within the binary itself.
